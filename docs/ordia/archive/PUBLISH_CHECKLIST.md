...
| Gate | Command / check | v0.9.1 status |
|---|---|---|
...
| **`narofitness-catalog.git`** | Reference app: `ordia.yaml` profile, `.cursor/` live, `docs/coordination/`, **pip** `ordia-core==0.9.1` |

Monorepo vendoring of `packages/ordia-*` in catalog is **removed** post-split.

---

## 3. PyPI — `ordia-core` 0.9.1

Package: `packages/ordia-core/pyproject.toml` (`name = ordia-core`, `version = 0.9.1`, **MIT**).

| Step | Action | Done |
|---|---|---|
| 3.1 | Version **0.9.1** aligned with CHANGELOG | [x] |
...
| 3.10 | Tag `ordia-core-v0.9.1` on ordia-package | [ ] |
...
- Publish `packages/ordia-cursor` v0.9.1 aligned with core.
...
| Step | Action | Done |
|---|---|---|
| 5.1 | `scripts/requirements-control.txt` → `ordia-core==0.9.1` | [ ] |
